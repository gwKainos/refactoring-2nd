...
describe("Book", () => {
  it("adds a reservation successfully", () => {
    ...
    book.addReservation(customer, false); // isPriority 추가
  ...
  it("allows adding multiple reservations", () => {
    ...
    book.addReservation(customer1, false); // 기본 우선순위
    book.addReservation(customer2, true);  // 높은 우선순위
  ...
  it("does not prevent duplicate reservations", () => {
    ...
    book.addReservation(customer, true); // 중복 허용
  ...
  it("returns a shallow copy of reservations", () => {
    ...
    book.addReservation(customer, true); // 우선순위 예약
